...
def load_cov():
    ...
    source = source.sample(n=10000)
    width = source.shape[1]
    class_anomaly = pd.DataFrame(columns=source.columns)
    class_normal = pd.DataFrame(columns=source.columns)
    for i in range(len(source)):
        label = source.iloc[i, width - 1]
        if label == 4 or label == 6 and len(class_anomaly) < 200:
            class_anomaly = class_anomaly.append(source.iloc[i, :])
        elif label == 2:
            class_normal = class_normal.append(source.iloc[i, :])
    source = pd.concat([class_normal, class_anomaly])
    source = source.sample(frac=1).reset_index(drop=True)
